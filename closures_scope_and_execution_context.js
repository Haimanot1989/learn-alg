...
function makeHistory(limit) {
  let recentArgs = [];
  return stringArg => {
    if (stringArg == "undo" && recentArgs.length == 0) {
      return "nothing to undo";
    }
    if (stringArg == "undo") {
      let undoneWord = recentArgs.pop();
      return `${undoneWord} undone`;
    }

    if (recentArgs.length >= limit) {
      recentArgs.shift();
    }

    recentArgs.push(stringArg);

    return `${stringArg} done`;
  };
}
...
//1. the first number in the array that was passed into blackjack
//    PLUS the sum of the two numbers passed in as arguments into the DEALER function, IF that sum is 21 or below, OR
...
// continue to take the next number in the array after the previous PLAYER function left off.
// You will just need to make sure the array has enough numbers for all the PLAYER functions.
function blackjack(array) {
  // array --> [1-11]
  let nextCardIndex = 0;
  let sumOfCards = array.reduce((a, b) => a + b);
  function dealer(arg1, arg2) {
    //both numbers
    let playerInvocationCount = 0;
    let sum = arg1 + arg2;

    if (sumOfCards + sum <= 21) {
      return () => {
    ...
    function player() {
      let busted = sum > 21;
      playerInvocationCount++;
      if (playerInvocationCount == 1) {
        return sum;
      }

      if (!busted) {
        let cardFromArray = array[nextCardIndex];
        sum += cardFromArray;
        sumOfCards -= cardFromArray;
        nextCardIndex++;
        return sum <= 21 ? sum : "bust";
      }
